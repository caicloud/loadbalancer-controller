...
func getHelmAnnotation(ann map[string]string, namespace, name string) map[string]string {
	if ann == nil {
...
func InsertHelmAnnotation(dp *appsv1.Deployment, namespace, name string) {
	dp.Annotations = getHelmAnnotation(dp.Annotations, namespace, name)
	dp.Spec.Template.Annotations = getHelmAnnotation(dp.Spec.Template.Annotations, namespace, name)
}
